...
namespace SKNIBot.Core.Commands.ModerationCommands
{
    ...
    public class SendToAllCommand : BaseCommandModule
    {
        ...
        public async Task SendToAll(CommandContext ctx, [Description("Rola do której mają zostać wysłane wiadomości.")] DiscordRole role, [RemainingText, Description("Treść wiadomości.")] string message)
        {
            ...
            await ctx.Channel.SendMessageAsync("To mała wiadomość dla ciebie, ale stos wiadomości dla mnie");
            await ctx.Channel.SendMessageAsync("Apokalipsa zaczyna się");
            await ctx.Channel.SendMessageAsync("Wyszukiwanie ofiar");

            var members = ctx.Guild.Members.Where(m => !m.Value.IsBot && m.Value.Roles.Contains(role));

            await ctx.Channel.SendMessageAsync($"Znaleziono {members.Count()} ofiar");

            await ctx.Channel.SendMessageAsync("Wysyłanie orędzia");

            var sentMessagesCount = 0;

            DiscordMessage progressBarMessage = await ctx.Channel.SendMessageAsync("`----------" + " 0 %`");
            ...
            await ctx.Channel.SendMessageAsync($"Zrobiłem swoje, teraz ty mierz się z gniewem ludu. Wysłano {sentMessagesCount} wiadomości.");
        }
